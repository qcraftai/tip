...
from .utils import get_corners, get_grid, get_local_map, objects2frame


class ClusterLoader(torchdata.Dataset):
    def __init__(
        self, nusc, nusc_maps, ego_only, flip_aug, is_train, t_spacing, only_y
    ):
        self.nusc = nusc
    ...
    def get_ixes(self):
        ...
        for scene in self.data:
            if not self.ego_only:
                names = [
                    name
                    for name in self.data[scene]
                    if (
                        self.data[scene][name]["k"].split(".")[0] == "vehicle"
            ...
            for name in names:
                ...
                for t in ts:
                    dist = np.linalg.norm(
                        self.data[scene][name]["interp"](t + self.local_ts[-1])[:2]
    ...
    def get_state(self, scene, name, t0):
        ...
        # map
        lmap = get_local_map(
            self.nusc_maps[self.scene2map[scene]],
            center,
            self.stretch,
            self.layer_names,
            self.line_names,
    ...
    def render(self, lmap, centerlw, lobjs, lws):
        ...
        for layer_name in self.layer_names:
            for poly in lmap[layer_name]:
                # draw the lines
                pts = np.round(
                    (poly - self.bx[:2] + self.dx[:2] / 2.0) / self.dx[:2]
                ).astype(np.int32)
                pts[:, [1, 0]] = pts[:, [0, 1]]
        ...
        pts = np.round((pts - self.bx[:2] + self.dx[:2] / 2.0) / self.dx[:2]).astype(
            np.int32
        )
        pts[:, [1, 0]] = pts[:, [0, 1]]
    ...
    def get_tgt(self, ltgt):
        pts = np.round(
            (ltgt[:, :2] - self.bx[:2] + self.dx[:2] / 2.0) / self.dx[:2]
        ).astype(np.int32)
        tgt = np.zeros((ltgt.shape[0], self.nx, self.ny))

        pts = np.concatenate(
            (np.arange(ltgt.shape[0], dtype=np.int32)[:, np.newaxis], pts), 1
        )
...
def compile_data(
    ...
    valloader = torchdata.DataLoader(
        valdata,
        batch_size=bsz,
        shuffle=False,
        num_workers=num_workers,
